...
![Tests: 4092 passing](https://img.shields.io/badge/Tests-4092%20passing-2E7D32)
...
- **Adaptive Explanation Depth** — single-rule explanations deepen based on the rule's history. First-time findings keep a concise structured explanation; recurring rules append category-specific root-cause guidance; worsening rules add a severity timeline and "what changed" wording. No LLM reasoning — depth is a deterministic function of history length, closed remediation cycles, and trend.
- **Adaptive Step-Outcome Reporting** — during a guided remediation session, report each manual step's result in plain English (`step 1 worked`, `step 2 failed with permission denied`, `it didn't work`). The agent updates the session timeline, classifies the failure category, and returns deterministic, command-aware guidance such as retrying with `sudo`, installing a missing dependency, or checking for a config-management tool that keeps re-applying the insecure setting. No LLM.
- **Provenance On Demand** — ask `prove FW-002`, `show evidence for it`, or `what triggered FW-002?` to get a deterministic evidence chain for any cached finding: scanner source/commands, raw evidence signals, cross-scanner validation, rule rationale, CIS/MITRE mappings, attack-chain membership, and per-rule history. If a known rule ID is not cached, the agent may run that single rule to refresh evidence. No LLM.
- **Audit History Slimming** — persists the latest 50 audit snapshots by default, keeping the newest 5 fully detailed and replacing older entries with slim summaries that preserve counts, findings, fingerprints, and scorecards. Keeps `audit-history.json` bounded while preserving diff comparisons and compliance trend charts.
...
- **Long-Horizon Category Coverage Tracking** — the agent remembers which of the 17 audit categories have been checked across sessions, surfaces blind spots after partial audits with a narrative coverage note, and offers one-click chips such as *Check filesystem security* to finish the review. Coverage survives restarts and is preserved through drift checks, verification re-audits, and category-filter fallback.
...
- **Agent-first Avalonia UI** — the Security Agent conversation is the permanent workspace, with compact suggested actions, inline progress/results, Context/Evidence/Runs inspector tabs, a focused-check launcher, markdown-rendered messages, copyable command rows, and five product-level navigation destinations backed by the existing capability views.
...
Verify a specific finding after remediation:
