...
2. Agent scanners collect firewall, port, service, SSH daemon configuration, file permissions, filesystem audit findings (world-writable files, SUID/SGID binaries, unowned files, sticky-bit checks, /tmp mount options), kernel and system hardening parameters, user accounts, shadow entries, password aging, PAM configuration, interface, route, and connection state from local Linux commands, plus data-source capability status for those commands.
