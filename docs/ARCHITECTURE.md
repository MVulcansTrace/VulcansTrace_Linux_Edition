...
- `MainViewModel` — receives `RemediationPlanBuilder` via constructor for evidence export (not created inline). Exposes five product-level destinations; `NavigationHubViewModel` groups the existing capability view models into Investigations, Policy & Intelligence, Operations, and System without duplicating their state.
- `Views/AgentView.axaml` — Agent-first workspace with a permanent transcript, compact suggested actions and state artifacts, a sticky composer, and Context/Evidence/Runs inspector tabs. `VtAgentAvatar` renders a code-native V + signal-trace glyph on Agent-owned surfaces, while the user composer remains visually neutral. `TracePulseRing` projects the real page/progress state around the Agent identity; the inspector becomes a drawer at compact widths; and the searchable Agent command catalog is exposed globally through `Ctrl+K`. Markdown, verification/remediation command rows, selected-finding actions, suggestions, history, remediation sessions, and the inline `/` palette remain available.
- `Services/MotionSettings` — presentation-only motion gate. Machine mode and `VT_REDUCED_MOTION=1` disable the Trace Pulse rotation, completion pulse, and state-artifact transitions without changing commands or state.
- `Views/NavigationHubView.axaml` — reusable tab host for capability views inside a product-level destination. The selected leaf label remains the machine-readable `AppStateNode.view` value used by automation.
