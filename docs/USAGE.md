...
- `Fix FW-001` — single-finding remediation preview when rollback guidance is present
- `Remediate FW-001` — persisted guided remediation session with verification and export
- `Verify remediation abc12345` — before/after verification for an active remediation session
...
**Guided Remediation Sessions** — When you type `remediate FW-001`, the agent creates a persisted manual session with a short session ID, a before snapshot, step state, and a Verify Remediation button. After you complete the manual steps, click **Verify Remediation** or type `verify remediation <session-id>` to re-run the original audit intent and produce a before/after diff. Blocked sessions remain visible with their safety reasons but cannot be verified as completed remediation. Use **Export Session** to save a markdown report for review or audit handoff.
