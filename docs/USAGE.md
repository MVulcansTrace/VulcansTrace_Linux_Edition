...
6. Review results:
   - **Investigations → Findings** — searchable, filterable table of all detected threats. The **MITRE ATT&CK** column shows mapped techniques for each finding.
   - **Investigations → Timeline** — visual timeline of findings by category, including Trace Map correlations and host grouping.
   - **Investigations → Incident Story** — flowing attack narrative generated from findings and correlations, including system posture and a time-ordered event narrative.
   - **Policy & Intelligence** — Rules, Threat Intel, Suppressions, Coverage, Compliance, and Risk share one destination with inner tabs.
   - **Operations** — Schedules, Notifications, and Live Stream share one destination.
   - **System** — Logs, Doctor diagnostics, and Analyst Action Log share one destination.
7. Use **Export Evidence** to save a cryptographically-signed ZIP bundle.

### Security Agent View

The Avalonia UI opens on a first-class **VulcansTrace Agent** workspace. The conversation remains visible while audits run and while result/error artifacts are shown. The right inspector is organized into **Context**, **Evidence**, and **Runs** rather than stacking every feature on one page. It can answer local posture questions such as:
...
- **Command palette** — press `Ctrl+K` from any destination to return to Agent and search the same command catalog. Use Up/Down to navigate, Enter to run, and Escape or `Ctrl+K` to close.
- **Suggested actions** — compact `Full audit`, `Analyze log`, and `Investigate target` actions start or prepare the three primary workflows without replacing the conversation.
- **Focused checks** — the crosshair menu in the composer owns Firewall, Ports, Services, Processes, and advanced scan options.
- **Context / Evidence / Runs inspector** — Context shows the current session and selected finding; Evidence contains pinned findings/messages; Runs contains baseline actions, audit comparison/history, and remediation sessions. At compact widths the same inspector becomes a dismissible drawer so the conversation keeps the available space.
- **Trace Pulse** — the Agent's V + signal-trace glyph, avatar ring, header line, status copy, and state artifact share one source of truth: ready, investigating, completed, or blocked. The ring uses real progress when the operation reports it. The glyph marks Agent-owned surfaces while the user composer remains neutral. Set `VT_REDUCED_MOTION=1` to keep the semantic colors and state copy while disabling optional Pulse motion.
- **Pinned chat messages** — pin any completed transcript message and review saved pins under **Evidence**. Pins persist when the user config directory is writable; otherwise the inspector shows that pins are session-only.
...
The Agent workspace keeps one home for each action class. The composer owns scan profile and focused checks; **Context** owns selected-finding explanation, verification, and pinning; **Runs** owns baselines, drift, history comparison, and remediation sessions; the **Share** menu owns audit/session exports; and Threat Intel import/export lives under **Policy & Intelligence → Threat Intel**. Remediation exports include an impact preview block, preconditions, backup commands, apply commands, rollback commands or hints, and verification commands; risky or unclassified apply/backup commands must have explicit rollback guidance before the plan can be exported. Session exports include session ID, step state, blocked reasons, before snapshot, remediation plan, verification diff when present, and a chronological timeline of session events. Audit history keeps the latest 50 lightweight snapshots by default; the newest 5 remain fully detailed and older retained entries are slimmed. Agent audit findings are loaded into **Investigations → Findings**. Coverage, Compliance, and Risk are available under **Policy & Intelligence**. The conversation continues to show the data-source capability report for scanner inputs such as iptables, nftables, ss, netstat, ip, systemctl, sshd, docker, crictl, ctr, and kubectl.
...
**Remediation Session Management** — **Agent → Runs** includes a persisted-session browser with refresh, resume, and delete actions. Chat commands `list my sessions` / `show sessions` list persisted sessions with their ID, status, rule ID, and creation time, and `resume session <id>` reloads a session into the conversation.
...
**Policy & Intelligence → Threat Intel** lists imported IOCs, supports type/search filtering, imports STIX or MISP JSON, removes individual IOCs, and clears the store. Imported IOCs are immediately available for correlation in the next audit or live stream session.
