...
6. Review results:
   ...
   - **Incident Story tab** — flowing attack narrative generated from findings and correlations. Shows the likely attack chain (e.g., "C2 → Lateral Movement → Privilege Escalation") and context-aware recommended responses first, then a **System Posture** section (point-in-time configuration findings from agent rules, grouped per category with finding counts) and a time-ordered **Timeline** of log events; sections with no findings are hidden. A **Copy Markdown** button exports the narrative with one click.
