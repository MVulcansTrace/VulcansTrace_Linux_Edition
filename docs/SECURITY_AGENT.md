...
- **Warning surfacing** — `JsonFileAgentMemoryStore.PersistenceWarning` is surfaced in the Avalonia UI as an info message, like the warnings from other stores. If a JSON file is corrupt or contains invalid entries, the file is quarantined to `<filename>.corrupt.<timestamp>.json` and the store starts fresh or loads only the valid rows; transient I/O failures leave the file in place and retry on the next start.
...
The Avalonia application opens on a first-class **VulcansTrace Agent** workspace. Its transcript remains visible while state artifacts change, and the surrounding application is organized into five product destinations: Agent, Investigations, Policy & Intelligence, Operations, and System. The view supports:
...
- **Suggested and focused actions** — compact Full audit, Analyze log, and Investigate target actions sit above the transcript; Firewall, Ports, Services, Processes, and advanced scan options share the composer crosshair menu.
- **Context / Evidence / Runs inspector** — current session and finding detail, pinned evidence/messages, and baseline/history/remediation work each have one contextual home.
- **Pinned chat messages** — any completed transcript message can be pinned from the chat bubble and reviewed under **Evidence**. Pins are persisted in the user config directory when available; if persistence falls back, the inspector shows that pins are session-only.
