...
- A guided, step-by-step UX with explicit preconditions, backup commands, apply commands, rollback commands, and verification commands gives the operator full control. A compact impact preview panel at the top of every remediation card summarizes the expected impact, rollback path, and verification command before the user reaches the copyable apply commands.
