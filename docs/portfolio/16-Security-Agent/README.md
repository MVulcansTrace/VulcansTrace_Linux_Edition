...
- **Rule-based posture checks** — evaluates firewall, port, service, SSH, file permission, filesystem audit, kernel hardening, network, cron job, package vulnerability, container, Kubernetes, and threat intel rules without external AI dependencies
...
- [SecurityRules](../../../VulcansTrace.Linux.Agent/Rules/SecurityRules) — firewall, network, service, port, SSH, file permission, filesystem audit, kernel hardening, user account, logging, cron job, package vulnerability, container, Kubernetes, and threat intel checks
