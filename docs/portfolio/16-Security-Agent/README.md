...
- **On-demand provenance** — `ShowEvidence` assembles a deterministic evidence chain for cached findings from scanner source/commands, raw evidence signals, cross-scanner validation, rule rationale, CIS/MITRE mappings, attack-chain membership, and per-rule history; known uncached rule IDs can run a single-rule refresh, but responses never use an LLM
- **Deterministic follow-up questions** — operates on the last audit result for changes since last audit, critical/high explanations, category filtering, prioritized remediation, interactive single-finding remediation with pre-flight impact simulation, suppressed listing, batch headless auto-fix with dry-run preview and policy-gated execution; on-demand evidence/provenance uses cached findings when available and can refresh a known uncached rule through the single-rule path
...
- **Audit history slimming** — retains the newest 50 audit snapshots by default, keeping the newest 5 fully detailed (capabilities, attack chains, rule results) and replacing older entries with slim summaries that preserve counts, findings, and scorecards so the on-disk file stays bounded
...
- **Remediation Session Management** — lists all persisted sessions with ID, status, rule ID, and creation time; supports refresh/resume/delete from the Agent view plus natural-language chat commands (`list my sessions`, `show sessions`, `resume session <id>`)
- **Avalonia Security Agent view** — first-class navigation view exposing chat, `/` slash-command palette, quick-action chips, markdown-rendered messages, copyable command rows with safety/structure badges, grouped and filterable findings, selected-finding explanations, user-friendly scanner warnings, privilege warnings, rule coverage totals, persistent selectable audit history diff with narrative summaries, baseline set/drift/show actions, suppression review actions, cancellation, audit export, guarded remediation preview export, and guided session timelines with success-only export tracking
...
- [ReferenceResolver.cs](../../../VulcansTrace.Linux.Agent/Dialogue/ReferenceResolver.cs) — focused-finding/category reference resolution shared by explanation and evidence
