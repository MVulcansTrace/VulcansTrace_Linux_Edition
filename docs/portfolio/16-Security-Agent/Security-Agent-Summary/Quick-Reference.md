...
| Auto-Fix CLI | `--auto-fix` | Batch auto-remediation; supports `--dry-run` to preview with risk before/after, command count, rollback availability, restart impact, and lockout risk per section, `--policy` (Conservative/Standard/Aggressive), `--allow-restart`, `--allow-packages`, and `--yes` to skip confirmation. Automatically skips risky commands missing rollback guidance. |
