...
| Escalation | Time-correlated Beaconing + LateralMovement from same host -> participating findings become Critical |
...
- **Correlation-based escalation** -- Medium severity for uncorrelated beaconing (filtered from output on Low intensity), Critical when LateralMovement from the same host confirms active attack progression
